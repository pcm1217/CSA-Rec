...
from models.CSA-Rec_CoT import *
from pre_train.utils_geo import data_partition, SeqDataset, SeqDataset_Inference
...
def train_model_phase1(args):
    print('CSA-Rec start train phase-1\n')
    if args.multi_gpu:
...
def train_model_phase2(args):
    print('CSA-Rec strat train phase-2\n')
    if args.multi_gpu:
        world_size = torch.cuda.device_count()
...
def train_model_phase1_(rank, world_size, args):
    ...
    # preprocess data
    dataset = data_partition(args.rec_pre_trained_data, path=f'')
    [user_train, user_valid, user_test, usernum, poinum, geonum] = dataset
    print('user num:', usernum, 'poi num:', poinum)
    ...
    train_data_set = SeqDataset(user_train, usernum, poinum, args.maxlen)
    if args.multi_gpu:
    ...
    for epoch in tqdm(range(epoch_start_idx, args.num_epochs + 1)):
        ...
        for step, data in enumerate(train_data_loader):
            ...
            u, seq, geo_seq, time_seq, pos, neg = u.numpy(), seq.numpy(), geo_seq.numpy(), time_seq.numpy(), pos.numpy(), neg.numpy()
            model([u,seq,geo_seq, time_seq, pos,neg], optimizer=adam_optimizer, batch_iter=[epoch,args.num_epochs + 1,step,num_batch], mode='phase1')  ##model²»Ò»Ñù
...
def train_model_phase2_(rank,world_size,args):
    ...
    for epoch in tqdm(range(epoch_start_idx, args.num_epochs + 1)):
        ...
        for step, data in enumerate(train_data_loader):
            u, seq, geo_seq, time_seq, pos, neg = data
            u, seq, geo_seq, time_seq, pos, neg = u.numpy(), seq.numpy(), geo_seq.numpy(), time_seq.numpy(), pos.numpy(), neg.numpy()
            model([u,seq,geo_seq,time_seq,pos,neg], optimizer=adam_optimizer, batch_iter=[epoch,args.num_epochs + 1,step,num_batch], mode='phase2')
            if step % max(10,num_batch//100) ==0:
                if rank ==0:
...
def inference_(rank, world_size, args):
    ...
    if usernum>11000:
        users = random.sample(range(1, usernum + 1), 11000)
    else:
    ...
    for u in users:
        if len(user_train[u]) < 1 or len(user_test[u]) < 1: 
            print(f"Skip user {u}, train={len(user_train[u])}, test={len(user_test[u])}")
            continue
        user_list.append(u)

    inference_data_set = SeqDataset_Inference(user_train, user_valid, user_test, user_list, poinum, args.maxlen)  
    ...
    for _, data in enumerate(inference_data_loader):
        u, seq, geo_seq, time_seq, pos, neg = data
        u, seq, geo_seq, time_seq, pos, neg = u.numpy(), seq.numpy(), geo_seq.numpy(), time_seq.numpy(), pos.numpy(), neg.numpy()
        model([u, seq, geo_seq, time_seq, pos, neg], mode='generate')