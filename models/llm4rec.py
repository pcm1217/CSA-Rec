...
import os
from transformers import AutoTokenizer, OPTForCausalLM, LlamaForCausalLM, LlamaTokenizer, PreTrainedTokenizerFast, AutoModelForCausalLM
...
class llm4rec(nn.Module):
    def __init__(
        ...
        if llm_model == 'llama':
            ...
            self.llm_tokenizer = PreTrainedTokenizerFast(tokenizer_file=os.path.join(local_path, "tokenizer.json"))
            """
            special_tokens = {
                'pad_token': '[PAD]',
                'bos_token': '</s>',
            ...
            self.llm_tokenizer = AutoTokenizer.from_pretrained(local_path)
